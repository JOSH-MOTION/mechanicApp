...
export const uploadAvatar = async (fileUri, userId) => {
  ...
  export const registerMechanic = async (mechanicname, mechanicEmail, mechanicId, latitude, longitude, contact) => {
    
    try {
        const mechanicId = ID.unique();
        const mechanic = await databases.createDocument(
            config.databaseId,
            config.mechaniccollectionId,
            mechanicId, // or ID.unique() if you want to generate a new ID
            {
